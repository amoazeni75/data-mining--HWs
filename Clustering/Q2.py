...
def preprocessing_baseline(list_of_sentences):
    ...
    Used for pre processing method B
    """
    processed_sentences = []
...
def preprocessing_stpwrd_remvl(list_of_sentences):
    ...
    Used for pre processing methods S1, S2, S3
    """
...
def preprocessing_neg_tag(list_of_sentences):
    ...
    Used for pre processing method N1, N2, N3
    """
...
def build_models(x_train, y_train, x_test, y_test, feature_range):
    ...
    for features in range(feature_range[0], feature_range[1], 500):
        ...
        r = {}
        r['features'] = features
        r['train_acc'], r['test_acc'], r['train_f1'], r['test_f1'], r['tr_cf'], r['te_cf'], _, _ = get_train_test_score(
            mnb,
            train_set,
            test_set,
            y_train,
...
df_train = pd.read_csv('train.csv', encoding=encoding)
df_test = pd.read_csv('test.csv', encoding=encoding)
df_train.head()
df_test.head()
